...
// #3
 ...
 describe("signed in user favoriting a post", () => {
    ...
    describe("POST /topics/:topicId/posts/:postId/favorites/:id/destroy", () => {
      ...
      it("should destroy a favorite", (done) => { //this test is actually failing.  remove the done();
        const options = {
          url: `${base}${this.topic.id}/posts/${this.post.id}/favorites/create`
        };
        ...
        request.post(options, (err, res, body) => {
          ...
          .then((favorites) => {
              console.log("favorites.length before delete....")
            ...
            request.post(`${base}${this.topic.id}/posts/${this.post.id}/favorites/${favorite.id}/destroy`,
              (err, res, body) => {
                this.post.getFavorites()
                .then((favorites2) => {
                    console.log("favorites.length after delete....")
                    console.log(favorites2.length)
                  expect(favorites2.length).toBe(favCountBeforeDelete - 1);
                  done();
                });
              }
            );
